...
from bnlearn.plot import plot, plot_graphviz, get_node_properties, get_edge_properties, hierarchical_layout
from bnlearn.plot import normalize_independence_frame
...
def independence_test(model, df, test="chi_square", alpha=0.05, prune=False, verbose=3):
    ...
    Plotting uses ``edge_filter='p_value'`` or ``'logp'``.
    ...
    if isinstance(model['model'], (DirectLiNGAM, ICALiNGAM)):
        ...
        model_update['independence_test'] = normalize_independence_frame(pd.DataFrame(results), test_name=test)
